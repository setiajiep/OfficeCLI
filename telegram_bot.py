...
def render_model_picker(user_id):
    ...
    text = (
        ...
        "║  🎯  *PILIH MODEL AI (AGY)*      ║\n"
        "╚══════════════════════════════╝\n\n"
        f"✅ *Model Aktif:* `{current_display}`\n\n"
        "Tap salah satu model di bawah untuk menggantinya.\n"
        "Model yang dipilih akan digunakan di semua instruksi AI berikutnya:"
    )
    ...
    for m in AVAILABLE_MODELS:
        ...
        btn_label = f"{prefix}{m['icon']} {m['name']}"
...
def delete_message(chat_id, message_id):
    ...
    try:
        ...
        print(f"Error deleting message {message_id}: {e}", file=sys.stderr)
        return None

USER_MENU_STATE = {}

def get_user_menu_state(user_id):
    str_id = str(user_id)
    if str_id not in USER_MENU_STATE:
        USER_MENU_STATE[str_id] = {
            'last_fm_msg_id': None,
            'last_menu_msg_id': None,
            'last_help_msg_id': None,
            'last_status_msg_id': None
        }
    return USER_MENU_STATE[str_id]
...
def render_file_manager(user_id, current_dir, page=1, notice=None):
    ...
    try:
        ...
        notice = (notice + f"\n\u274c Error membaca folder: {e}") if notice else f"\u274c Error membaca folder: {e}"
    ...
    page_files = files[start_idx : start_idx + FILES_PER_PAGE]

    rel_path = current_dir.replace("/root", "~")

    # --- Teks Header ---
    text  = "\U0001f4c2 *FILE MANAGER \u2014 VPS EXPLORER*\n"
    text += "\u2501" * 30 + "\n"
    text += f"\U0001f4cd *Path*      : `{rel_path}`\n"
    text += f"\U0001f4be *Free Disk* : `{free_gb:.2f} GB`\n"
    text += f"\U0001f4c1 *Folder*    : `{len(folders)}`   \U0001f4c4 *File* : `{total_files}`"
    if total_pages > 1:
        text += f"   \U0001f4cc Hal `{page}/{total_pages}`"
    text += "\n"
    if notice:
        text += f"\n{notice}\n"
    text += "\u2501" * 30 + "\n"
    text += "\U0001f4a1 Upload foto/dokumen + caption instruksi \u2192 langsung diproses AI!"
    ...
    if parent_dir != current_dir and current_dir != "/root":
        nav_row.append({"text": "\u2b06\ufe0f Folder Atas", "callback_data": f"fm_cd:{parent_key}:1"})
    nav_row.append({"text": "\U0001f504 Refresh", "callback_data": f"fm_cd:{dir_key}:{page}"})
    root_key = encode_path("/root")
    nav_row.append({"text": "\U0001f3e0 Root", "callback_data": f"fm_cd:{root_key}:1"})
    inline_keyboard.append(nav_row)

    # --- Aksi Folder ---
    inline_keyboard.append([
        {"text": "\u2795 Folder Baru",    "callback_data": "fm_action:create_folder_prompt"},
        {"text": "\U0001f4dd File Baru",    "callback_data": "fm_action:create_file_prompt"},
        {"text": "\U0001f4e6 Zip Folder",  "callback_data": "fm_action:zip_cwd"}
    ])

    # --- Shortcut Navigasi ---
    myproject_key = encode_path("/root/MyProject")
    inline_keyboard.append([
        {"text": "\U0001f5c2\ufe0f MyProject",  "callback_data": f"fm_cd:{myproject_key}:1"},
        {"text": "\U0001f4ca Status VPS",  "callback_data": "fm_action:view_status"},
        {"text": "\U0001f4c8 Chart",       "callback_data": "fm_action:view_chart"},
        {"text": "\u26a1 Proses",          "callback_data": "fm_action:view_procs"}
    ])

    # --- Sesi & Backup ---
    session_mode = get_session_mode(user_id)
    if session_mode == "continue":
        session_text = "\U0001f4ac Sesi: Lanjut"
    elif session_mode == "new":
        session_text = "\U0001f195 Sesi: Baru"
    else:
        session_text = f"\U0001f516 Sesi: {session_mode[:8]}"

    inline_keyboard.append([
        {"text": "\U0001f4e6 Backup VPS",  "callback_data": "fm_action:do_backup"},
        {"text": session_text,              "callback_data": "fm_action:toggle_session_mode"},
        {"text": "\U0001f4dc Pilih Sesi",  "callback_data": "fm_action:list_sessions"}
    ])

    # --- Toggle Hidden ---
    toggle_hidden_text = "\U0001f648 Sembunyikan System" if show_hidden else "\U0001f441\ufe0f Tampilkan System"
    inline_keyboard.append([
    ...
    # --- Daftar Folder ---
    if folders:
        inline_keyboard.append([{"text": "\u2500\u2500\u2500 \U0001f4c1 FOLDER \u2500\u2500\u2500", "callback_data": "fm_action:noop"}])
        row = []
        for item in folders[:12]:
            full_path = os.path.join(current_dir, item)
            f_key = encode_path(full_path)
            # Hitung jumlah item di dalam folder
            try:
                sub_count = len(os.listdir(full_path))
                count_txt = f" ({sub_count})"
            except Exception:
                count_txt = ""
            btn_text = f"\U0001f4c1 {item[:18]}{count_txt}"
    ...
    # --- Daftar File ---
    if page_files:
        inline_keyboard.append([{"text": f"\u2500\u2500\u2500 \U0001f4c4 FILE (Hal {page}/{total_pages}) \u2500\u2500\u2500", "callback_data": "fm_action:noop"}])
        file_row = []
        for item in page_files:
            ...
            # Icon per tipe
            icon = "\U0001f4c4"
            if ext == ".pdf" or item.lower().startswith("doc-"): icon = "\U0001f4d5"
            elif ext in [".docx", ".doc"]:                       icon = "\U0001f4d8"
            elif ext in [".pptx", ".ppt"]:                       icon = "\U0001f4d9"
            elif ext in [".xlsx", ".xls", ".csv"]:              icon = "\U0001f4ca"
            elif ext in [".png", ".jpg", ".jpeg", ".webp", ".bmp"]: icon = "\U0001f5bc\ufe0f"
            elif ext in [".zip", ".tar", ".gz", ".rar"]:        icon = "\U0001f4e6"
            elif ext in [".py", ".sh", ".js", ".ts"]:           icon = "\U0001f4bb"
            elif ext in [".mp4", ".mkv", ".avi"]:               icon = "\U0001f3ac"
            elif ext in [".mp3", ".wav", ".ogg"]:               icon = "\U0001f3b5"
            ...
            try:
                ...
                if size_bytes < 1024:
                    ...
                    size_txt = f"{size_bytes/1024:.0f}K"
                else:
                    size_txt = f"{size_bytes/(1024*1024):.1f}M"
            except Exception:
                size_txt = ""

            btn_text = f"{icon} {item[:15]} ({size_txt})"
    ...
    if total_pages > 1:
        ...
        pag_row.append({"text": f"\U0001f4c4 {page}/{total_pages}", "callback_data": "fm_action:noop"})
        if page < total_pages:
            pag_row.append({"text": "Next \u25b6\ufe0f", "callback_data": f"fm_cd:{dir_key}:{page+1}"})
        inline_keyboard.append(pag_row)

    inline_keyboard.append([{"text": "❌ Tutup File Manager", "callback_data": "fm_action:close_fm"}])
                    ...
                    for fname in files:
                        ...
                        if os.path.isfile(full_fpath):
                            ...
                            if full_fpath not in before_files or mtime > (before_files.get(full_fpath, 0) + 0.01):
                                new_created_files.append(full_fpath)
            except Exception:
                pass

        # Also scan files mentioned directly in AGY output text
        raw_paths = re.findall(r'(?:file://)?(/root/[^\s\)\"\'>]+|[a-zA-Z0-9_\-\./]+\.(?:pdf|docx|xlsx|pptx|png|jpg|jpeg|webp|md|txt|py|sh|csv|json|zip))', output)
        for p in raw_paths:
            clean_p = p.replace("file://", "").rstrip(".,;:)")
            target_f = clean_p if clean_p.startswith("/") else os.path.join(work_dir, clean_p)
            if os.path.exists(target_f) and os.path.isfile(target_f) and target_f not in new_created_files:
                bname = os.path.basename(target_f)
                if bname not in SYSTEM_FILES and not bname.startswith("."):
                    new_created_files.append(target_f)

        # Also check brain artifact directory for generated output files
        brain_dir = "/root/.gemini/antigravity-cli/brain"
            ...
            try:
                for root_path, dirs, files in os.walk(brain_dir):
                    for fname in files:
                        ...
                        if ext in ['.png', '.jpg', '.jpeg', '.webp', '.pdf', '.docx', '.xlsx', '.pptx', '.txt', '.md', '.csv', '.json', '.zip']:
                            full_p = os.path.join(root_path, fname)
                            try:
                                if os.path.getmtime(full_p) >= (start_time - 5) and full_p not in new_created_files:
                                    new_created_files.append(full_p)
                            except Exception:
                                pass
            except Exception:
                pass

        # Build inline keyboard for created files if any
        file_buttons = []
        if new_created_files:
            row = []
            for fpath in new_created_files[:6]:
                fl_key = encode_path(fpath)
                bname  = os.path.basename(fpath)
                ext    = os.path.splitext(bname)[1].lower()

                if ext in [".png", ".jpg", ".jpeg", ".webp", ".bmp"]: icon = "🖼️"
                elif ext == ".pdf": icon = "📕"
                elif ext in [".docx", ".doc"]: icon = "📘"
                elif ext in [".xlsx", ".xls", ".csv"]: icon = "📊"
                elif ext in [".pptx", ".ppt"]: icon = "📙"
                elif ext in [".zip", ".tar", ".gz", ".rar"]: icon = "📦"
                elif ext in [".py", ".sh", ".js", ".ts"]: icon = "💻"
                else: icon = "📄"

                btn_text = f"📤 Kirim {icon} {bname[:16]}"
                row.append({"text": btn_text, "callback_data": f"send_file_tg:{fl_key}"})
                if len(row) == 2:
                    file_buttons.append(row)
                    row = []
            if row:
                file_buttons.append(row)

            dir_key = encode_path(work_dir)
            file_buttons.append([{"text": "📂 Buka di File Manager", "callback_data": f"fm_cd:{dir_key}:1"}])

        reply_markup = {"inline_keyboard": file_buttons} if file_buttons else None

        edit_message(chat_id, status_msg_id, full_output, reply_markup=reply_markup, parse_mode="HTML")
...
def process_callback_query(cq):
    ...
    if data == "close_msg":
        answer_callback_query(cq_id, "🙈 Ditutup")
    ...
    if data == "menu_help":
        ...
        help_txt = (
            ...
            "╚══════════════════════════════╝\n\n"
            "🤖 *AI & SESI*\n"
            "  • Kirim teks bebas → langsung diproses AGY AI\n"
            "  • Kirim *Voice Note* → otomatis ditranskripsi & dieksekusi\n"
            "  • `/model` → pilih model AI (Gemini / Claude / GPT)\n"
            "  • `/sessions` → riwayat & pilih sesi percakapan\n"
            "  • `/continue` `/new` → ubah mode sesi\n\n"
            "💻 *SISTEM & VPS*\n"
            "  • `/exec <cmd>` → jalankan perintah bash\n"
            "  • `/status` → dashboard RAM / CPU / Disk\n"
            "  • `/chart` → grafik analitik real-time\n"
            ...
            "  • `/cd` `/pwd` `/mkdir` `/rm` `/rename`\n"
            "  • Upload foto/dokumen + *caption instruksi* → diproses AI\n\n"
            "🌐 *WEB & NETWORK*\n"
            "  • `/web <url>` → baca & scrape halaman web\n"
            "  • `/ping <host>` → ping dari VPS\n"
            "  • `/port <host> <port>` → cek port terbuka\n"
            "  • `/ip <host>` → info geolokasi IP/domain\n"
    ...
    if data.startswith("fm_cd:"):
        ...
        try:
            ...
            sub_folders = sum(1 for x in sub_items if os.path.isdir(os.path.join(folder_path, x)))
            sub_files   = sum(1 for x in sub_items if os.path.isfile(os.path.join(folder_path, x)))
            total_size  = sum(
                os.path.getsize(os.path.join(r, f))
                for r, _, fs in os.walk(folder_path) for f in fs
            )
            size_mb = total_size / (1024 * 1024)
            stats_text = f"  ├ Sub-folder : `{sub_folders}`\n  ├ File       : `{sub_files}`\n  └ Total Size : `{size_mb:.2f} MB`"
        except Exception:
            stats_text = "  └ _(tidak dapat membaca statistik)_"

        rel_path = folder_path.replace("/root", "~")
        text  = "📁 *FOLDER MANAGER*\n"
        text += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        text += f"📌 *Nama*   : `{folder_name}`\n"
        ...
        btn = {"inline_keyboard": [
            [{"text": "📂 Buka / Masuk Folder",      "callback_data": f"fm_cd:{target_key}:1"}],
            [{"text": "📕 Convert Folder → PDF",      "callback_data": f"fm_folder_to_pdf:{target_key}"},
             {"text": "📦 Zip Folder",                "callback_data": f"fm_action:zip_item:{target_key}"}],
            [{"text": "🗑️ Hapus Folder",             "callback_data": f"fm_rm_confirm:{target_key}"}],
            [{"text": "🔙 Kembali ke Manager",        "callback_data": f"fm_cd:{parent_key}:1"}]
        ]}
        ...
        if os.path.exists(file_path) and os.path.isfile(file_path):
            ...
            size_txt = f"{size_mb:.2f} MB" if size_kb > 1024 else f"{size_kb:.1f} KB"
            ...
            if is_image:
                type_icon, type_label = "🖼️", "GAMBAR / FOTO"
            elif ext == ".pdf":
                type_icon, type_label = "📕", "DOKUMEN PDF"
            elif ext in [".docx", ".doc"]:
                type_icon, type_label = "📘", "DOKUMEN WORD"
            elif ext in [".xlsx", ".xls", ".csv"]:
                type_icon, type_label = "📊", "SPREADSHEET / CSV"
            elif ext in [".pptx", ".ppt"]:
                type_icon, type_label = "📙", "PRESENTASI"
            elif ext in [".zip", ".tar", ".gz", ".rar"]:
                type_icon, type_label = "📦", "FILE ARSIP"
            elif ext in [".py", ".sh", ".js", ".ts"]:
                type_icon, type_label = "💻", "SCRIPT / CODE"
            elif ext in [".mp4", ".mkv", ".avi"]:
                type_icon, type_label = "🎬", "FILE VIDEO"
            elif ext in [".mp3", ".wav", ".ogg"]:
                type_icon, type_label = "🎵", "FILE AUDIO"
            else:
                type_icon, type_label = "📄", "FILE"

            rel_path = file_path.replace("/root", "~")
            text  = f"{type_icon} *{type_label} — FILE MANAGER*\n"
            text += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            text += f"📌 *Nama*       : `{file_name}`\n"
            text += f"📍 *Path*       : `{rel_path}`\n"
            text += f"📦 *Ukuran*     : `{size_txt}`\n"
            text += f"🕒 *Diubah*     : `{mtime}`\n"
            ...
            if is_image:
                try:
                    ...
                    with Image.open(file_path) as img:
                        text += f"📐 *Resolusi*   : `{img.width} × {img.height} px`\n"
                        text += f"🎨 *Mode Warna* : `{img.mode}`\n"
                except Exception:
                    pass
        ...
        elif action in ["close_fm", "fm_close"]:
            answer_callback_query(cq_id, "🙈 File Manager ditutup")
            delete_message(chat_id, message_id)
            st = get_user_menu_state(user_id)
            if st.get('last_fm_msg_id') == message_id:
                st['last_fm_msg_id'] = None
            return
        ...
        if os.path.exists(file_path):
            answer_callback_query(cq_id, "📤 Mengirim file ke Telegram...")
            ext   = os.path.splitext(file_path)[1].lower()
            fname = os.path.basename(file_path)

            if ext in [".png", ".jpg", ".jpeg", ".webp", ".bmp"]:
                send_photo(chat_id, file_path, caption=f"🖼️ Hasil Foto: {fname}")
            elif ext in [".mp4", ".mkv", ".avi"]:
                send_video(chat_id, file_path, caption=f"🎬 Hasil Video: {fname}")
            elif ext in [".mp3", ".wav", ".ogg"]:
                send_audio(chat_id, file_path, caption=f"🎵 Hasil Audio: {fname}")
            elif ext == ".pdf":
                send_document(chat_id, file_path, caption=f"📕 Hasil PDF: {fname}")
            elif ext in [".docx", ".doc"]:
                send_document(chat_id, file_path, caption=f"📘 Hasil Dokumen: {fname}")
            elif ext in [".xlsx", ".xls", ".csv"]:
                send_document(chat_id, file_path, caption=f"📊 Hasil Spreadsheet: {fname}")
            elif ext in [".pptx", ".ppt"]:
                send_document(chat_id, file_path, caption=f"📙 Hasil Presentasi: {fname}")
            else:
                send_document(chat_id, file_path, caption=f"📄 Hasil File: {fname}")
        else:
            answer_callback_query(cq_id, "❌ File tidak ditemukan", show_alert=True)

def setup_bot_commands():
    commands = [
        {"command": "start",     "description": "🏠 Dashboard Utama & Status VPS"},
        {"command": "menu",      "description": "📱 Tampilkan Menu Keyboard"},
        {"command": "model",     "description": "🤖 Pilih Model AI (Gemini / Claude / GPT)"},
        {"command": "sessions",  "description": "📜 Riwayat & Pilih Sesi Percakapan"},
        {"command": "continue",  "description": "💬 Mode Sesi Lanjut (sambung chat lama)"},
        {"command": "new",       "description": "🆕 Mode Sesi Baru (mulai dari awal)"},
        {"command": "fm",        "description": "📂 File Manager Interaktif"},
        {"command": "exec",      "description": "💻 Jalankan Perintah Bash di VPS"},
        {"command": "status",    "description": "📊 Status RAM / CPU / Disk VPS"},
        {"command": "chart",     "description": "📈 Grafik Analitik Real-time VPS"},
        {"command": "top",       "description": "⚡ Process Manager & Monitor PID"},
        {"command": "services",  "description": "🛠️ Status Layanan Systemd"},
        {"command": "backup",    "description": "📦 Backup VPS ke GitHub & Telegram"},
        {"command": "web",       "description": "🌐 Baca & Scrape Halaman Web"},
        {"command": "yt",        "description": "🎬 Download Video (YouTube/IG/TikTok)"},
        {"command": "mp3",       "description": "🎵 Download Audio MP3 dari Video"},
        {"command": "qr",        "description": "📲 Buat QR Code dari Teks / URL"},
        {"command": "short",     "description": "🔗 Persingkat URL (TinyURL)"},
        {"command": "ping",      "description": "📡 Ping Host dari VPS"},
        {"command": "port",      "description": "🔌 Cek Port Terbuka di Host"},
        {"command": "ip",        "description": "🌍 Info Geolokasi IP / Domain"},
        {"command": "cuaca",     "description": "🌤️ Info Cuaca Kota Manapun"},
        {"command": "bandwidth", "description": "🚀 Test Kecepatan / Bandwidth VPS"},
        {"command": "remind",    "description": "⏰ Set Pengingat (contoh: /remind 10m Rapat)"},
        {"command": "calc",      "description": "🧮 Kalkulator Pintar (sin, cos, sqrt)"},
...
def get_main_menu_keyboard():
    return {
        "keyboard": [
            ...
            # ── Baris 2: File & Sesi ──
            [{"text": "📂 File Manager"}, {"text": "📜 Riwayat Sesi"}],
            [{"text": "💬 Sesi Lanjut"}, {"text": "🆕 Sesi Baru"}],
            # ── Baris 3: Monitor VPS ──
...
def process_update(update):
    ...
    # Define menu trigger strings for auto-deleting trigger messages
    menu_triggers = [
        "/start", "/menu", "📱 Tombol Menu", "📱 Menu",
        "fm", "/fm", "ls", "/ls", "browse", "/browse", "📂 File Manager",
        "/closemenu", "/hidemenu", "/close", "🙈 Sembunyikan Menu",
        "/help", "/bantuan", "❓ Bantuan",
        "/model", "/models", "/selectmodel", "🤖 Pilih Model AI", "🎯 Pilih Model AI",
        "📜 Riwayat Sesi", "📜 Pilih Sesi", "📜 List Sesi", "/sessions",
        "/status", "/sys", "/vps", "📊 Status VPS",
        "/chart", "/syschart", "📈 Chart VPS",
        "/top", "/ps", "/procs", "⚡ Top Processes", "⚡ Top Proses",
        "/services", "/service", "🛠️ Services", "🛠️ Layanan",
        "💬 Sesi Lanjut", "💬 Sesi Lanjut / Baru", "💬 Mode Sesi", "🆕 Sesi Baru",
        "🤖 Tanya AGY AI", "/ask", "/agy",
        "💻 Jalankan Bash", "💻 Exec Bash", "💻 Run Bash", "/exec_help",
        "📦 Backup VPS", "/backup", "/backupvps",
        "🌤️ Cuaca", "📲 QR Code", "🔗 Short URL", "🎬 Unduh Video", "🎵 Unduh MP3",
        "📡 Ping", "🌍 IP Info", "🧮 Kalkulator", "🚀 Bandwidth", "🌐 Web Reader"
    ]
    ...
    if text in ["/start", "/menu", "📱 Tombol Menu"]:
        u_st = get_user_menu_state(user_id)
        if u_st.get('last_menu_msg_id'):
            delete_message(chat_id, u_st['last_menu_msg_id'])
            u_st['last_menu_msg_id'] = None

        st = get_system_status()
        disk_free  = st.get('disk_free_gb', 0)
        ram_used   = st.get('mem_used_mb', 0)
        ram_total  = st.get('mem_total_mb', 1)
        ram_pct    = st.get('mem_percent', 0)
        cpu_load   = st.get('cpu_load', 'N/A')
        uptime_str = st.get('uptime', 'N/A')
        active_model_name = get_model_display_name(get_user_model(user_id))
        session_mode = get_session_mode(user_id)
        session_label = "Lanjut" if session_mode == "continue" else ("Baru" if session_mode == "new" else f"Custom ({session_mode[:8]}...)") 

        filled = int(ram_pct / 10)
        ram_bar = "█" * filled + "░" * (10 - filled)
        ...
        menu_text = (
            ...
            f"  │            `{ram_used:.0f} MB / {ram_total:.0f} MB`\n"
            f"  └ Disk Free: `{disk_free:.2f} GB tersedia`\n\n"
            "🤖 *KONFIGURASI AI*\n"
            f"  ├ Model Aktif : `{active_model_name}`\n"
            f"  ├ Mode Sesi   : `{session_label}`\n"
            f"  └ Working Dir : `{current_cwd.replace('/root', '~')}`\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "💡 *CARA PAKAI BOT INI:*\n"
            "  • Ketik pesan bebas → *AGY AI akan mengeksekusi*\n"
            "  • Kirim *Voice Note* → otomatis ditranskripsi ke AI\n"
            "  • Upload foto/dokumen + *caption instruksi* → langsung diproses\n"
            "  • `/exec <cmd>` → jalankan perintah bash di VPS"
        )

        inline_dash = {"inline_keyboard": [
            [
                {"text": "🤖 Tanya AGY AI",   "callback_data": "menu_ask_agy"},
                {"text": "🎯 Pilih Model AI", "callback_data": "menu_select_model"}
            ],
            [
                {"text": "📂 File Manager",   "callback_data": "fm_cd:L3Jvb3Q=:1"},
                {"text": "📊 Status VPS",     "callback_data": "menu_sys_status"}
            ],
            [
                {"text": "⚡ Top Proses",     "callback_data": "fm_action:view_procs"},
                {"text": "📦 Backup VPS",     "callback_data": "menu_backup"}
            ],
            [
                {"text": "❓ Bantuan & Panduan", "callback_data": "menu_help"}
            ],
            [
                {"text": "❌ Tutup Dashboard", "callback_data": "close_msg"}
            ]
        ]}
        send_message(chat_id, menu_text, reply_markup=get_main_menu_keyboard(), parse_mode="Markdown")
        sent = send_message(chat_id, "⚡ *AKSI CEPAT:*", reply_markup=inline_dash, parse_mode="Markdown")
        if sent and len(sent) > 0:
            u_st['last_menu_msg_id'] = sent[-1]["message_id"]
        return
    ...
    if text in ["/closemenu", "/hidemenu", "/close", "🙈 Sembunyikan Menu"]:
        ...
        if u_st.get('last_menu_msg_id'):
            delete_message(chat_id, u_st['last_menu_msg_id'])
            u_st['last_menu_msg_id'] = None
        if u_st.get('last_fm_msg_id'):
            delete_message(chat_id, u_st['last_fm_msg_id'])
            u_st['last_fm_msg_id'] = None

        close_btn = {"inline_keyboard": [[{"text": "❌ Tutup", "callback_data": "close_msg"}]]}
        send_message(
            chat_id,
    ...
    if text.lower() in ["fm", "/fm", "ls", "/ls", "browse", "/browse"] or text == "📂 File Manager":
        ...
        msg_text, reply_markup = render_file_manager(user_id, current_cwd, page=1)
        sent = send_message(chat_id, msg_text, reply_markup=reply_markup)
        if sent and len(sent) > 0:
            u_st['last_fm_msg_id'] = sent[-1]["message_id"]
        return

    if text in ["📜 Riwayat Sesi", "📜 Pilih Sesi", "📜 List Sesi"]:
        msg_text, reply_markup = render_session_picker(page=1)
        send_message(chat_id, msg_text, reply_markup=reply_markup, parse_mode="Markdown")
        return

    # (handled above in unified session block)

    if text in ["💬 Sesi Lanjut", "💬 Sesi Lanjut / Baru", "💬 Mode Sesi"]:
        new_mode = toggle_session_mode(user_id)
        mode_label = "💬 *Sesi Lanjut* — Percakapan disambung dari sesi sebelumnya." if new_mode == "continue" else "🆕 *Sesi Baru* — Setiap pesan memulai percakapan baru."
        mode_icon  = "💬" if new_mode == "continue" else "🆕"
        send_message(chat_id,
            f"{mode_icon} *MODE SESI DIPERBARUI*\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
            f"{mode_label}\n\n"
            f"💡 Gunakan tombol *Riwayat Sesi* untuk memilih sesi percakapan tertentu.",
            parse_mode="Markdown")
        return

    if text in ["🆕 Sesi Baru"]:
        set_user_session_id(user_id, "new")
        send_message(chat_id,
            "🆕 *SESI BARU DIAKTIFKAN*\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "Setiap pesan akan memulai percakapan AGY AI yang baru dari awal.",
            parse_mode="Markdown")
        return
    ...
    if text in ["🤖 Tanya AGY AI", "/ask", "/agy"]:
        send_message(
            chat_id,
            "🤖 *ANTIGRAVITY AI (AGY) — SIAP DIGUNAKAN!*\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "Cukup ketik instruksi atau pertanyaan Anda secara bebas,\n"
            "atau kirim *Voice Note* — AGY AI akan langsung merespons.\n\n"
            "*Contoh penggunaan:*\n"
            "  • `Buatkan file Python untuk scraping web`\n"
            "  • `Analisis disk usage di VPS`\n"
            "  • `Jelaskan cara deploy aplikasi Node.js`\n\n"
            "📍 Working Dir: `" + current_cwd.replace('/root', '~') + "`",
            parse_mode="Markdown"
        )
        return

    if text in ["💻 Jalankan Bash", "💻 Exec Bash", "💻 Run Bash", "/exec_help"]:
        send_message(
            chat_id,
            "💻 *EKSEKUSI PERINTAH BASH — VPS TERMINAL*\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            "  • `/exec df -h`\n"
            "  • `/exec uptime && free -m`\n"
            "  • `/exec systemctl status antigravity-bot`",
            parse_mode="Markdown"
        )
        return
    ...
    if text in ["/status", "/sys", "/vps", "📊 Status VPS"]:
        u_st = get_user_menu_state(user_id)
        if u_st.get('last_status_msg_id'):
            delete_message(chat_id, u_st['last_status_msg_id'])
            u_st['last_status_msg_id'] = None
        ...
        close_btn = {"inline_keyboard": [[{"text": "❌ Tutup Status", "callback_data": "close_msg"}]]}
        sent = send_message(chat_id, status_text, reply_markup=close_btn, parse_mode="Markdown")
        if sent and len(sent) > 0:
            u_st['last_status_msg_id'] = sent[-1]["message_id"]
        return
    ...
    if text in ["/top", "/ps", "/procs", "⚡ Top Processes", "⚡ Top Proses"]:
        procs = get_top_processes(limit=8)
        msg = (
            "⚡ *TOP PROSES — MEMORY & CPU MONITOR*\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        )
        btn_rows = []
        for i, p in enumerate(procs, 1):
            msg += f"`{i:2}.` `PID {p['pid']:6}` *{p['name'][:18]:18}* | 🧠 `{p['mem_mb']:6.1f} MB` | ⚡ `{p['cpu']:5.1f}%`\n"
            btn_rows.append([{"text": f"🛑 Kill PID {p['pid']} — {p['name'][:15]}", "callback_data": f"proc_kill:{p['pid']}"}])

        btn_rows.append([{"text": "🔄 Refresh Daftar Proses", "callback_data": "fm_action:view_procs"}])
        send_message(chat_id, msg, reply_markup={"inline_keyboard": btn_rows}, parse_mode="Markdown")
        return

    if text in ["/services", "/service", "🛠️ Services", "🛠️ Layanan"]:
    ...
    if text in ["/help", "/bantuan", "❓ Bantuan"]:
        u_st = get_user_menu_state(user_id)
        if u_st.get('last_help_msg_id'):
            delete_message(chat_id, u_st['last_help_msg_id'])
            u_st['last_help_msg_id'] = None
        ...
        close_btn = {"inline_keyboard": [[{"text": "❌ Tutup Panduan", "callback_data": "close_msg"}]]}
        sent = send_message(chat_id, help_txt, reply_markup=close_btn, parse_mode="Markdown")
        if sent and len(sent) > 0:
            u_st['last_help_msg_id'] = sent[-1]["message_id"]
        return
